    ...
    case 'checkout':
        ...
        switch ($act) {
            ...
            case 'save':
                ...
                break;
            case 'pay':
                $id = isset($_GET['MaHD']) ? $_GET['MaHD'] : null;
                $controller_obj->pay($id);
                break;
            case 'execute':
                $id = isset($_GET['transId']) ? $_GET['transId'] : null;
                $method = isset($_GET['method']) ? $_GET['method'] : null;
                if ($method ==="momo") {
                    $MaHD = isset($_GET['extraData']) ? $_GET['extraData'] : null;
                } else {
                    $MaHD = isset($_GET['id']) ? $_GET['id'] : null;
                }
                $controller_obj->execute($id, $method,$MaHD);
            case 'order_complete':
                $id = isset($_GET['MaHD']) ? $_GET['MaHD'] : null;
                $controller_obj->order_complete($id);
                break;
            default:
                $controller_obj->list();
                break;
